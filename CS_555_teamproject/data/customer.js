...
let create = async (firstname, lastname, mailid,  phoneNumber) =>{
    let newCustomer = {
      firstname,
      lastname,
      mailid,
    ...
    if(stringCheck(firstname))throw "error with first name"
    if(stringCheck(lastname))throw "error with last name"
    


    const customer = await customers();
    const insertInfo = await customer.insertOne(newCustomer);
    if (!insertInfo.acknowledged || !insertInfo.insertedId) throw 'Could not add a band';
...
const update = async (id,name,genre,website,recordCompany,groupMembers,yearBandWasFormed) => {
  ...
  if(typeof(yearBandWasFormed)!="number") throw "the input year band was formed is not a number"
  if(!(yearBandWasFormed>=1900&&yearBandWasFormed<=2023)) throw "the input year is not in a correct range"
